...
// Cap how many items we keep per persona to bound React state + memory.
...
export default function Home() {
  ...
  const start = useCallback(
    (targetUrl: string, targetGoal: string) => {
      ...
      // The server emits `fatal` right before closing the stream. We close the
      // EventSource ourselves to prevent Chrome's default auto-reconnect from
      // spawning a brand-new cohort run on every transient hiccup.
      ...
      es.onerror = () => {
        // If the connection is fully dead, close and report. Do NOT let
        // EventSource auto-reconnect — each reconnect would spawn another run.
        if (es.readyState === EventSource.CLOSED) {
          setError("Stream disconnected.");
          return;
        }
        // Transient: close immediately and surface the error so we don't loop.
        es.close();
        sourceRef.current = null;
